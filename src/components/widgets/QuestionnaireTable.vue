    ...
    >
      <div v-if="openRightUploadDrawer">
        <DrawerUploadFile
          ...
          :title="DrawerTitle"
          :detail="DrawerRemark"
          :uploadImage="f_upload_image"
          :uploadFile="f_upload_file"
          :additionalMessage="f_additional_message"
          :addressUrl="f_address_url"
        />
      </div>
    </Drawer>
    <Drawer
      ...
      for (let index = 0; index < result.length; index++) {
        ...
        if (f_question_group == element.group) {
          ...
          if (this.ShowScoreWT == 1) {
            ...
            document.getElementById(IdSumTitle).innerHTML =
              element.score +
              "/" +
              dataSet.f_questionWtSub +
              "*" +
              dataSet.f_questionWtMain;
          } else {
            document.getElementById(IdSum).innerHTML =
